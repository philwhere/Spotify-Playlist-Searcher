...
{
    public class SpotifyClientWithSemaphore : ISpotifyClient
    {
        ...
        public SpotifyClientWithSemaphore(IOptions<SpotifyClientConfiguration> configuration, HttpClient httpClient, SpotifyRequestPagingCalculator pagingCalculator)
        {
        ...
        private async Task<SpotifyItemResponse<TItem>> GetAllPages<TItem>(string firstPageUrl, string accessToken, int parallelRequestSize) where TItem : SpotifyItem
        {
            ...
            var pages = await GetPagesInParallel<TItem>(accessToken, remainingUrls, parallelRequestSize);
        ...
        private async Task<List<SpotifyItemResponse<TItem>>> GetPagesInParallel<TItem>(string accessToken,
            IEnumerable<string> urls, int parallelRequestSize) where TItem : SpotifyItem
        {
            var pages = new ConcurrentBag<SpotifyItemResponse<TItem>>();
            var semaphore = new SemaphoreSlim(parallelRequestSize);
            ...
            {
                {
                    ...
                    try
                    {
                        var page = await _httpClient.GetWithToken<SpotifyItemResponse<TItem>>(url, accessToken);
                        pages.Add(page);
                    }
                    finally
                    {
